...
- **OAuth server (`mcp()` plugin in `auth.ts`)**: makes the app the OAuth 2.1
  ...
  login need — dynamic client registration, PKCE, authorize/token/refresh
...
- **No consent screen — the redirect allowlist is what replaces it.**
  ...
  claude.com and loopback, and refuses any URI containing a comma — better-auth
...
- **Continuing an OAuth login:** a logged-out authorize bounces to
  ...
  origin). The plugin's after-login hook may answer the sign-in fetch with
  that 302 itself, which fetch cannot follow cross-origin — so both auth pages
  treat "error, but `getSession` now says signed in" as success.
